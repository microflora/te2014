...
$(document).ready(function() {
	...
	function updateStatus(cell, params, status) {
		//alert("update_translator_status?__EVENTTARGET=" + status + "&__EVENTARGUMENT=" + params);
		$.get("update_translator_status?__EVENTTARGET=" + status + "&__EVENTARGUMENT=" + params, 
...
<table width="98%" border="0" cellspacing="1" cellpadding="6"
	...
	<tr class="DataTableTH">
		...
		<th valign="middle">Translator</th>
	...
	<tr class="<?php echo $temp; ?>">
		...
		<?php if ($SelectedSID == $item['SID']) {?> <select name="speakerid"
			...
			<?php if ($translatorid == 0) echo "selected"; ?>>Select Speaker</option>
			<?php foreach($speakers as $speaker):?>
			<option value="<?php echo $speaker['id']; ?>"
			<?php if ($translatorid == $speaker['id']) echo "selected"; ?>><?php echo $speaker['LastName'].", ".$speaker['FirstName'];?></option>
			<?php endforeach; ?>
		...
		<td align="left" valign="middle"
			...
			<?php $count = 0;
			foreach ($item['Translators'] as $item2):
			$count = $count + 1;
			if ($count == 1) { ?>
